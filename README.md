...
* [`src/cljs/affiliate_ui/`](src/cljs/affiliate_ui/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/affiliate_ui/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `affiliate-ui` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/affiliate_ui/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns affiliate-ui.example
  (:require [affiliate-ui.config :as config])
