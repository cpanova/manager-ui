      ...
      (let [db (:db cofx)
           ...
           (case active-panel
                 ...
                 ; :offers-panel {:db set-panel
                                ; :dispatch [:manager-ui.offers.events/init-offers]
                                ; :dispatch [:manager-ui.offers.events/load-offers]}
                 :stats-panel {:db set-panel
                               :dispatch [:manager-ui.stats.events/set-active-report (:report params)]}
                 ; :offer-panel {:db set-panel
                 ;               :dispatch-n [
                 ;                            ; [:manager-ui.offer.events/init (:id params)]]}
                 ;                            [:manager-ui.offer.events/load-offer (:id params)]
                 ;                            [:manager-ui.offer.events/load-tracking-link (:id params)]]}
                 :login-panel {:db set-panel
                               :dispatch [:manager-ui.login.events/reset]}
                 ; :sign-up-panel {:db set-panel
                 ;                 :dispatch [:manager-ui.sign-up.events/reset]}
                 {:db set-panel}))))
