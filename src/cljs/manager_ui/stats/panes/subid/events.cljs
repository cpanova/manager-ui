(ns affiliate-ui.stats.panes.subid.events
  (:require
   [re-frame.core :as rf]
   [ajax.core :as ajax]
...
(rf/reg-event-db
  ::init
  (fn [db _]
    (assoc db state-key affiliate-ui.stats.panes.subid.db/defaults)))
  ...
  (fn [cofx [_ subid]]
      (let [auth (:local-store cofx)
           ...
           {:http-xhrio  {:method          :get
                          :uri             (str cfg/URL "/affiliate/stats/sub/" subid "/")
                          :params          params
                          :headers         {"Authorization" (str "JWT " (:token auth))}
                          :response-format (ajax/json-response-format {:keywords? true})
                          :on-success      [::load-subid-stats-success]
                          :on-failure      [:http-fail]}})))


(rf/reg-event-db
  ::load-subid-stats-success
  (fn [db [_ data]]
